...
<div class="py-12">
        <div class="max-w-7x1 mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-2">
                    <form method="POST" action="{{route('spremi_knjige')}}">
                        @csrf
                        ...
                        <!--<div class="mt-4">
                            <x-label for="autorID" value="{{ __('Autor') }}" />
                            <select id="autorID" name="autorID" class="block mt-1 w-full" required>
                        ...
                        <div class="mt-4">
                            <x-label for="policaID" value="{{ __('Polica') }}" />
                            <select id="policaID" name="policaID" class="block mt-1 w-full" required>
                            <option value="">Odaberite policu</option>
                            @foreach($polica as $polica)
                                <option value="{{ $polica->id }}">{{ $polica->ime }}</option>
                            @endforeach
                            </select>
                        </div>
